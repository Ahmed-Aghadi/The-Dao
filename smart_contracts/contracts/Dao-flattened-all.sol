...
library Leb128 {
    ...
    function encodeUnsignedLeb128FromUInt64(
        uint64 value
    ) internal pure returns (Buffer.buffer memory result) {
        while (true) {
...
library FilecoinCBOR {
    ...
    function readCid(
        bytes memory cborData,
        uint byteIdx
    ) internal pure returns (CommonTypes.Cid memory, uint) {
        uint8 maj;
        uint value;
    ...
    function readChainEpoch(
        bytes memory rawResp,
        uint byteIdx
    ) internal pure returns (CommonTypes.ChainEpoch, uint) {
        int64 tmp = 0;
    ...
    function writeDealLabel(
        CBOR.CBORBuffer memory buf,
    ...
    function deserializeDealLabel(
        bytes memory rawResp
    ) internal pure returns (CommonTypes.DealLabel memory) {
        uint byteIdx = 0;
        CommonTypes.DealLabel memory label;
    ...
    function readDealLabel(
        bytes memory rawResp,
        uint byteIdx
    ) internal pure returns (CommonTypes.DealLabel memory, uint) {
        uint8 maj;
        uint len;

        (maj, len, byteIdx) = CBORDecoder.parseCborHeader(rawResp, byteIdx);
        require(
            maj == MajByteString || maj == MajTextString,
            "invalid maj (expected MajByteString or MajTextString)"
        );
...
library MarketCBOR {
    ...
    function serializePublishStorageDealsParams(
        ...
        for (uint64 i = 0; i < params.deals.length; i++) {
            ...
            capacity += Misc.getBytesSize(
                params.deals[i].proposal.storage_price_per_epoch.serializeBigInt()
            );
            capacity += Misc.getBytesSize(
                params.deals[i].proposal.provider_collateral.serializeBigInt()
            );
            capacity += Misc.getBytesSize(
                params.deals[i].proposal.client_collateral.serializeBigInt()
            );
    ...
    function deserializeMarketDealNotifyParams(
        bytes memory rawResp
    ) internal pure returns (MarketTypes.MarketDealNotifyParams memory ret) {
        uint byteIdx = 0;
        uint len;
    ...
    function serializeDealProposal(
        ...
        uint256 capacity = 0;
        bytes memory storage_price_per_epoch = dealProposal
    ...
    function deserializeDealProposal(
        bytes memory rawResp
    ) internal pure returns (MarketTypes.DealProposal memory ret) {
        uint byteIdx = 0;
        uint len;
        bytes memory tmp;
...
interface ITablelandTables {
     ...
     */
    function create(address owner, string memory statement) external payable returns (uint256);
     ...
     */
    function mutate(address caller, uint256 tableId, string calldata statement) external payable;
...
library TablelandDeployments {
    ...
    address internal constant OPTIMISM = 0xfad44BF5B843dE943a09D4f3E84949A11d3aa3e6;
    // TablelandTables address on Arbitrum One.
    address internal constant ARBITRUM = 0x9aBd75E8640871A5a20d3B4eE6330a04c962aFfd;
    // TablelandTables address on Arbitrum Nova.
    address internal constant ARBITRUM_NOVA = 0x1A22854c5b1642760a827f20137a67930AE108d2;
    ...
    address internal constant OPTIMISM_GOERLI = 0xC72E8a7Be04f2469f8C2dB3F1BdF69A7D516aBbA;
    // TablelandTables address on Arbitrum Goerli.
    address internal constant ARBITRUM_GOERLI = 0x033f69e8d119205089Ab15D340F5b797732f646b;
    // TablelandTables address on Polygon Mumbai.
    address internal constant MATICMUM = 0x4b48841d4b32C4650E4ABc117A03FE8B51f38F68;
    // TablelandTables address on Filecoin Calibration.
    address internal constant FILECOIN_CALIBRATION = 0x030BCf3D50cad04c2e57391B12740982A9308621;

    // TablelandTables address on for use with https://github.com/tablelandnetwork/local-tableland.
    address internal constant LOCAL_TABLELAND = 0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512;
...
library SQLHelpers {
    ...
    function toInsert(
        ...
        return
            string(abi.encodePacked("INSERT INTO ", name, "(", columns, ")VALUES(", values, ")"));
    }
    ...
    function toUpdate(
        ...
        return string(abi.encodePacked("UPDATE ", name, " SET ", setters, filter));
    }
    ...
    function toDelete(
        ...
        string memory name = toNameFromId(prefix, tableId);
        return string(abi.encodePacked("DELETE FROM ", name, " WHERE ", filters));
    }
